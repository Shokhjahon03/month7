...
const AllCardsPG = ({x,setX,setParam}) => {
  ...
  let [search,setSearch]=useState({val:''})
  let [vv,setVv]=useState([])
  ...
  let searchv=()=>{
    let news=foods.filter((e)=>e.foodname.toLowerCase().includes(search.val.toLowerCase()))
    setVv(news)
  }
  useEffect(()=>{
    searchv()
  ...
  return (
    <>
      ...
      <div className='container'>
        <div className='mt-[156px] w-full'>
          <input value={search.val} onChange={(e)=>setSearch({val:e.target.value})} className=' border pl-[50px] w-full h-[40px] rounded-[10px] bg-slate-100 outline-0' type="text" placeholder='Search Food' />
        </div>
      {
        ...
        </div>:<div className=' w-full flex sm:flex  flex-wrap justify-center gap-y-[41.67px] gap-x-[21.07px] mt-[106px] border-b pb-[57.67px]'>
        {
         search? <>{
           vv.map((e,i)=>(
